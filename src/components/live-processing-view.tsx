  ...
  return (
    <section aria-labelledby="processing-title">
      <header className={ui.screenHead}>
        <div>
          <p className={ui.eyebrow}>run {run.id.slice(0, 7)} / {run.kind ?? "IMPORT"}</p>
          <h1 className={ui.sectionTitle} id="processing-title">
            {needsConfiguration ? "Select an application root." : "Processing durable evidence."}
          </h1>
        </div>
        <p className="m-0 text-muted">
          <code>repository {repositoryId.slice(0, 7)}</code>
          <br />
          <code>{run.step === "FETCH_COMMITS" ? run.fetchedCommits : run.processedCommits} / {run.expectedCommits ?? "?"} commits</code>
          <br />
      ...
      {run.status === "FAILED" ? (
        <div className={`${ui.alert} mt-4`} role="alert">
          <strong>{run.kind === "REFRESH" ? "Refresh failed." : "Import failed."}</strong>
          <p>{run.error?.message ?? "The processing run could not complete."}</p>
          <p>
            {run.kind === "REFRESH"
              ? "The previous successful snapshot remains available and unchanged."
              : "No staged output was activated."}
          </p>
          {run.error?.code ? (
            <p className="font-mono text-xs">
              Error code: <code className="break-all">{run.error.code}</code>
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="mt-8 grid grid-cols-[minmax(0,1fr)_20rem] gap-8 max-[800px]:grid-cols-1">
        <div>
          {needsConfiguration ? (
            <AppRootConfiguration
              candidates={run.appRootCandidates}
              configuring={configuring}
              kind={run.kind}
              onChange={setSelectedAppRoot}
              onSubmit={configureAppRoot}
              selectedAppRoot={selectedAppRoot}
            />
          ) : (
            <ol
              ...
              {PROCESSING_STEPS.map((step, index) => {
                ...
                return (
                  <li
                    ...
                    className={`grid grid-cols-[3rem_minmax(0,1fr)_auto] gap-4 border-b border-soft p-4 font-mono text-xs last:border-b-0 max-[560px]:grid-cols-1 ${state === "active" ? "bg-[#1d1a12]" : ""}`}
                    key={step.key}
                  >
                    ...
                    >
                      {getStepValue(run, step.key, state)}
                    </span>
                  </li>
                );
              })}
          ...
          <p className="mt-3 font-mono text-xs text-muted" role="status" aria-live="polite">
            {configuring
              ? "Saving application root."
              : retrying
                ? "Retry requested. Waiting for the worker to claim this run."
        ...
        <aside className={ui.terminal}>
          ...
          <div className={ui.terminalBody}>
            {needsConfiguration ? (
              <>
                <div>{run.appRootCandidates.length} {run.appRootCandidates.length === 1 ? "root" : "roots"} discovered</div>
                <div>snapshot unchanged</div>
              </>
            ) : (
              <>
                <div>attempt {run.attemptCount}</div>
                <div>step {run.step}</div>
                <div>worker {run.worker.status.toLowerCase()}</div>
                <div>
                  {run.worker.heartbeatAgeSeconds === null
                    ? "heartbeat unavailable"
                    : `heartbeat ${Math.round(run.worker.heartbeatAgeSeconds)}s ago`}
                </div>
                <div>{run.warnings?.length ?? 0} warnings recorded</div>
                {run.nextAttemptAt ? <div>next attempt {new Date(run.nextAttemptAt).toLocaleString()}</div> : null}
              </>
            )}
          </div>
          <div className="flex flex-col gap-3 px-4 pb-4">
            {canCancel || canRetry ? (
              <button
                aria-disabled={cancelling || retrying}
                className={`${ui.button} aria-disabled:cursor-wait aria-disabled:opacity-60`}
                onClick={() => {
                  if (cancelling || retrying) return;
                  if (canCancel) void cancelRun();
                  else void retryRun();
                }}
                type="button"
              >
            ...
            <Link className={ui.primaryButton} href={`/repositories/${repositoryId}`}>
              {run.status === "SUCCEEDED" ? "View repository" : "Open repository"}
            </Link>
          </div>
        </aside>
      </div>
